...
`brand/logo-1024.png` (square brand mark for Google Business Profile, social
avatars, etc.) is generated by:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\make-logo.ps1
```

The glyph is kept inside a centre safe zone because Google crops profile logos
to a circle.
